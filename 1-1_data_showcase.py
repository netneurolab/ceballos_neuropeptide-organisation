...
from netneurotools.networks import strength_preserving_rand_sa, randmio_und
...
if os.path.exists('data/sc_nulls_Schaefer400_TianS4_bin.npy'):
    sc_nulls = np.load('data/sc_nulls_Schaefer400_TianS4_bin.npy')
else:
    itr = 1000
    nulls = Parallel(n_jobs=32) \
            (delayed(randmio_und)(sc, itr) 
    ...
    np.save('data/sc_nulls_Schaefer400_TianS4_bin.npy', sc_nulls)
...
# %% 10% longest connections
# get 10% longest connections
distance = np.load('data/template_parc-Schaefer400_TianS4_desc-distance.npy')
distance = (sc * distance)[triu_indices]

# get 10% longest connections
longest_connections = np.percentile(distance, 90)
mask = distance > longest_connections

# get cge for longest connections
cge_longest = cge_receptors[triu_indices][mask]
cge_rest = cge_receptors[triu_indices][~mask]

# remove zeros and nans
cge_longest = cge_longest[cge_longest != 0]
cge_rest = cge_rest[cge_rest != 0]
cge_longest = cge_longest[~np.isnan(cge_longest)]
cge_rest = cge_rest[~np.isnan(cge_rest)]

# get cge for longest connections in nulls
cge_longest_nulls = []
cge_rest_nulls = []

for null in sc_nulls:
    mask_null = (null * distance)[triu_indices] > longest_connections

    cge_longest_null = cge_receptors[mask_null]
    cge_rest_null = cge_receptors[~mask_null]

    cge_longest_nulls.append(cge_longest_null)
    cge_rest_nulls.append(cge_rest_null)
